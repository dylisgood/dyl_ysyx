...
static void exec_once(Decode *s, vaddr_t pc) {
  ...
  static int kong = 0;
  int j = 0;
  if( ((s->isa.inst.val & 0xef) == 0xef) || ((s->isa.inst.val & 0x0e7) == 0x0e7) \
         ){  //jal && x1 || jalr && !x1
      for (int i = 0; i < jj; i++) {
        Elf64_Sym *sym = &symbols[i];
        if(sym->st_info == 18){
  ...
  else if( ((s->isa.inst.val & 0x00008067) == 0x00008067) || ((s->isa.inst.val & 0x00078067) == 0x00078067) ){  //jalr && x0 || jal && x0 
    for (int i = 0; i < jj; i++) {
        Elf64_Sym *sym = &symbols[i];
        if(sym->st_info == 18){
        if(s->pc >= sym->st_value && s->pc <= (sym->st_value + sym->st_size))
