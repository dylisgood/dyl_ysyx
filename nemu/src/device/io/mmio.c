...
void add_mmio_map(const char *name, paddr_t addr, void *space, uint32_t len, io_callback_t callback) {
  ...
  if (in_pmem(left) || in_pmem(right)) { //if overlap with mem
    report_mmio_overlap(name, left, right, "pmem", PMEM_LEFT, PMEM_RIGHT);
  }
  for (int i = 0; i < nr_map; i++) {    //if overlap with other device map
...
word_t mmio_read(paddr_t addr, int len) {
  #ifdef CONFIG_DTRACE
  printf("device read: device name: %s \n",fetch_mmio_map(addr)->name);
  #endif
...
void mmio_write(paddr_t addr, int len, word_t data) {
  #ifdef CONFIG_DTRACE
    printf("device write: device name: %s,   addr = %x\n",fetch_mmio_map(addr)->name,addr);
  #endif
