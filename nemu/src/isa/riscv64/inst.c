...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("0000000 ????? ????? 001 ????? 01110 11", sllw   , R, R(dest) = BITS( BITS(src1,31,0) << src2, 31, 0));
  INSTPAT("0000000 ????? ????? 001 ????? 00110 11", slliw  , I, R(dest) = BITS( BITS(src1,31,0) << imm,  31, 0 ) );
  INSTPAT("0100000 ????? ????? 101 ????? 00110 11", sraiw  , I, sword_t src = BITS(src1,31,0); R(dest) = src >> BITS(imm,4,0));
