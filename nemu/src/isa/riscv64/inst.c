...
#define immI() do { *imm = SEXT(BITS(i, 31, 20), 12); } while(0)  //已经符号位扩展了
...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("??????? ????? ????? 000 ????? 11000 11", beqz   , B, s->dnpc = ((src1 == src2) ? (s->pc + (imm<<1)) : s->snpc));//BEQ
  INSTPAT("??????? ????? ????? 001 ????? 11000 11", bne    , B, s->dnpc = (src1 != src2) ? (s->pc + (imm<<1)) : s->snpc);
  INSTPAT("??????? ????? ????? 100 ????? 11000 11", blt    , B, sword_t sr1 = src1; sword_t sr2 = src2; s->dnpc = (sr1 < sr2) ? (s->pc + (imm<<1)) : s->snpc);
  INSTPAT("??????? ????? ????? 101 ????? 11000 11", bge    , B, sword_t sr1 = src1; sword_t sr2 = src2; s->dnpc = (sr1 >= sr2) ? (s->pc + (imm<<1)) : s->snpc);
  ...
  INSTPAT("0000000 ????? ????? 010 ????? 01100 11", slt    , R, sword_t sr1 = src1; sword_t sr2 = src2; R(dest) = sr1 < sr2 ? 1 : 0 );
  INSTPAT("??????? ????? ????? 010 ????? 00100 11", slti   , I, sword_t sr1 = src1; sword_t imm2 = imm; R(dest) = sr1 < imm2 ? 1: 0 );
  INSTPAT("0000000 ????? ????? 011 ????? 01100 11", snez   , R, R(dest) = (src1 < src2) ? 1 : 0); //sltu
  ...
  INSTPAT("0100000 ????? ????? 101 ????? 01100 11", sra    , R, sword_t src = src1; R(dest) = src >> BITS(src2,5,0));
  INSTPAT("010000? ????? ????? 101 ????? 00100 11", srai   , I, sword_t src = src1; R(dest) = src >> BITS(imm,5,0)); //
  ...
  INSTPAT("0100000 ????? ????? 101 ????? 01110 11", sraw   , R, int32_t src = BITS(src1,31,0); R(dest) = SEXT( src >> BITS(src2,4,0),32) );
  INSTPAT("0000000 ????? ????? 001 ????? 00110 11", slliw  , I, R(dest) = SEXT( BITS(src1 << imm,31,0), 32) );//先左移 再截断 再扩展
  INSTPAT("0000000 ????? ????? 101 ????? 00110 11", srliw  , I, R(dest) = SEXT( BITS(src1,31,0) >> imm, 32) ); //先截断 再右移 再扩展
  INSTPAT("0100000 ????? ????? 101 ????? 00110 11", sraiw  , I, int32_t src = BITS(src1,31,0); R(dest) = SEXT((src >> BITS(imm,4,0)),32));
...
/*INSTPAT("0000001 ????? ????? 001 ????? 01100 11", mulh   , R, sword_t rs1=src1; sword_t rs2=src2; R(dest) = BITS(rs1 * rs2,127,64));
  INSTPAT("0000001 ????? ????? 010 ????? 01100 11", mulhsu , R, sword_t rs1=src1; word_t rs2=src2; R(dest) = BITS(rs1 * rs2,127,64));
  INSTPAT("0000001 ????? ????? 010 ????? 01100 11", mulhu ,  R, R(dest) = BITS(src1 * src2,127,64)); */
  INSTPAT("0000001 ????? ????? 100 ????? 01100 11", div    , R, sword_t rs1=src1; sword_t rs2=src2; R(dest) = rs1 / rs2);
  INSTPAT("0000001 ????? ????? 101 ????? 01100 11", divu   , R, R(dest) = src1 / src2);
  INSTPAT("0000001 ????? ????? 110 ????? 01100 11", rem    , R, sword_t rs1=src1; sword_t rs2=src2; R(dest) = rs1 % rs2);
  INSTPAT("0000001 ????? ????? 111 ????? 01100 11", remu   , R, R(dest) = src1 % src2);

  INSTPAT("0000001 ????? ????? 000 ????? 01110 11", mulw   , R, R(dest) = SEXT(BITS(src1,31,0) * BITS(src2,31,0), 32););
  INSTPAT("0000001 ????? ????? 100 ????? 01110 11", divw   , R, int32_t rs1=BITS(src1,31,0); int32_t rs2=BITS(src2,31,0); R(dest) = SEXT(BITS(rs1 / rs2,31,0),32));
  ...
  INSTPAT("??????? ????? ????? 110 ????? 11100 11", csrrsi , I, R(dest) = sr(sr_num), sr(sr_num) = sr(sr_num) | sr_imm);
  INSTPAT("??????? ????? ????? 111 ????? 11100 11", csrrci , I, R(dest) = sr(sr_num), sr(sr_num) = sr(sr_num) & ~sr_imm);

  INSTPAT("0000000 00000 00000 000 00000 11100 11", ecall  , N, s->dnpc = cpu.sr[0], isa_raise_intr(11, s->pc));  //not support timer_imterrupt
  INSTPAT("0011000 00010 00000 000 00000 11100 11", mret   , N, s->dnpc = cpu.sr[1] );
  
  INSTPAT("0000000 00001 00000 000 00000 11100 11", ebreak , N, NEMUTRAP(s->pc, R(10))); // R(10) is $a0

  INSTPAT("??????? ????? ????? 001 ????? 00011 11", fencei , I,                       ); //for nanos-lite
