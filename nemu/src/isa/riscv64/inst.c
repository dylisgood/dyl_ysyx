...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("??????? ????? ????? 000 ????? 11011 11", ret    , I, s->dnpc = ((src1 + imm) & 0xfffffffe); R(dest) = s->snpc);//jalr
  ...
  INSTPAT("??????? ????? ????? 010 ????? 00100 11", slti   , I, sword_t sr1 = src1; R(dest) = sr1 < imm ? 1: 0 );
  ...
  INSTPAT("0100000 ????? ????? 101 ????? 01110 11", sraw   , R, int32_t src = BITS(src1,31,0); R(dest) = src >> BITS(src2,4,0));
  ...
  INSTPAT("0000001 ????? ????? 100 ????? 01100 11", rem    , R, sword_t rs1=src1; sword_t rs2=src2; R(dest) = rs1 % rs2);
  INSTPAT("0000001 ????? ????? 101 ????? 01100 11", remu   , R, R(dest) = src1 % src2);
