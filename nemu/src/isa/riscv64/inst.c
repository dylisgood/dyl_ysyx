...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  //INSTPAT("0000000 0???? ????? 001 ????? 00110 11", slliw  , R, R(dest) = SEXT(src1 << src2 , 32));
  ...
  INSTPAT("010000? ????? ????? 101 ????? 00100 11", srai   , I, int src = src1; R(dest) = src >> BITS(imm,5,0)); //
  INSTPAT("??????? ????? ????? 100 ????? 00000 11", lbu    , I, R(dest) = Mr(src1 + imm, 1));
  INSTPAT("??????? ????? ????? 111 ????? 00100 11", andi   , I, R(dest) = SEXT(BITS(imm,11,0),12) & src1);
  INSTPAT("0000000 ????? ????? 001 ????? 01110 11", sllw   , R, src1 = src1 << BITS(rs2,4,0) ;R(dest) = BITS(src1, 31, 0));
